...
@pytest.fixture
def mock_auth_headers(test_user_id: str) -> dict:
    """模拟认证头（使用真实 JWT token）"""
    token = create_access_token(data={"sub": test_user_id})
    return {"Authorization": f"Bearer {token}"}
...
@pytest.fixture
def sample_document(test_db, test_user_id, test_user):
    """创建示例文档"""
    ...
    doc = Document(
        ...
        file_size=1024000,
        file_path="/data/uploads/test_doc.pptx",
        page_count=5,
...
@pytest.fixture
def sample_outline(test_db, test_user_id, test_user):
    """创建示例大纲"""
